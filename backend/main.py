...
async def get_todays_meal_plan(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        # Look for today's meal plan or the most recent adaptive plan
        todays_plan = None
        for plan in meal_plans:
            ...
            if plan_date:
                try:
                    ...
                    if plan_date_obj == today:
                        todays_plan = plan
                        break
                except:
                    continue
        
        # If no plan for today, get the most recent plan and adapt it
        if not todays_plan and meal_plans:
            latest_plan = meal_plans[0]
            
            # Get today's consumption to adapt the plan
            consumption_data = await get_user_consumption_history(current_user["email"], limit=10)
            today_consumption = [
                record for record in consumption_data
                if datetime.fromisoformat(record.get("timestamp", "").replace("Z", "+00:00")).date() == today
            ]
            
            # Create adapted plan for today
            todays_plan = await adapt_meal_plan_based_on_consumption(
                current_user["email"], 
                today_consumption, 
                latest_plan
            )
        
        # If still no plan, create a basic one based on user profile
        if not todays_plan:
            user_data = await get_user_by_email(current_user["email"])
            profile = user_data.get("profile", {})
            health_conditions = profile.get("medicalConditions", [])
            
            # Create basic meal plan based on health conditions
            basic_meals = {
                "breakfast": "Oatmeal with berries and nuts (heart-healthy, condition-friendly)",
                "lunch": "Grilled chicken salad with olive oil dressing (balanced nutrition)",
                "dinner": "Baked salmon with steamed vegetables (omega-3, low sodium)",
                "snack": "Greek yogurt with almonds (protein-rich, satisfying)"
            }
            
            # Customize based on conditions
            if any("diabetes" in condition.lower() for condition in health_conditions):
                basic_meals["breakfast"] = "Steel-cut oats with cinnamon and walnuts (low glycemic)"
                basic_meals["snack"] = "Apple slices with almond butter (fiber + protein)"
            
            if any("hypertension" in condition.lower() or "blood pressure" in condition.lower() for condition in health_conditions):
                basic_meals["lunch"] = "Quinoa bowl with vegetables (low sodium, potassium-rich)"
                basic_meals["dinner"] = "Herb-crusted fish with sweet potato (DASH diet friendly)"
            
            if any("heart" in condition.lower() for condition in health_conditions):
                basic_meals["breakfast"] = "Oatmeal with walnuts and berries (heart-healthy)"
                basic_meals["dinner"] = "Grilled salmon with asparagus (omega-3 rich)"
            
            todays_plan = {
                "id": f"basic_{current_user['email']}_{int(datetime.utcnow().timestamp())}",
                "date": today.isoformat(),
                "type": "basic_adaptive",
