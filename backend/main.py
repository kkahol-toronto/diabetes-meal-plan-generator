...
async def generate_shopping_list(
    ...
    try:
        ...
        print(recipes)
        prompt = f"""Generate a shopping list based on the following recipes:\n{json.dumps(recipes, indent=2)}\n\nGroup items by category (e.g., Produce, Dairy, Meat, etc.) and combine quantities for the same items. Provide quantities for solid items using grams (g) or kilograms (kg). For liquid items, use milliliters (ml) or liters (l). For spices and very small quantities of liquids, use teaspoons (tsp) or tablespoons (tbsp), but prefer ml/l for larger liquid amounts. Avoid including descriptive counts like 'medium' or 'large'. Only use other count-based units (like 'pieces' or 'bunches') if a weight, volume, or teaspoon/tablespoon unit is truly not suitable for the item or quantity. Prioritize standard units (kg, g, l, ml, tsp, tbsp).\nFormat the response as a JSON array of shopping items with the following structure:\n[\n    {{\n        \"name\": \"Item Name\",\n        \"amount\": \"Quantity with Standard Unit\",\n        \"category\": \"Category Name\"\n    }}\n]\n"""
        print("Prompt for OpenAI:")
        print(prompt)
