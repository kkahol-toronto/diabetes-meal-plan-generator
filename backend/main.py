...
async def get_todays_meal_plan(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        # Try to find an existing plan dated today
        todays_plan = None
        ...
        # If none, adapt the latest plan
        if not todays_plan and meal_plans:
            latest_plan = meal_plans[0]

            # Get today's consumption for adapting
            ...
            todays_plan = await adapt_meal_plan_based_on_consumption(
                current_user["email"], today_consumption, latest_plan
            )

        # If still none, build a basic template based on profile
        if not todays_plan:
            ...
            basic_meals = {
                ...
                "lunch": "Grilled tofu salad with olive oil dressing (balanced nutrition)",
                "dinner": "Mixed veggie stir-fry with quinoa (high fiber, low sodium)",
            ...
            todays_plan = {
                ...
                "meals": basic_meals,
                "created_at": datetime.utcnow().isoformat(),
                "notes": "Basic meal plan autogenerated."
            }
